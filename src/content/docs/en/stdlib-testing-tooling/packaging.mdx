...
> This lesson has no Run button — packaging and publishing must run locally: `uv init --lib mypackage && cd mypackage && uv build`.
...
`requires-python` is the floor you promise to support — pip refuses to install the package on anything
older, so set it to the oldest interpreter you actually test against. Note that `distutils` was removed
in Python 3.12: a build backend such as `hatchling` (above), `setuptools`, or `uv_build` is now required,
and `setup.py` is no longer the entry point.
